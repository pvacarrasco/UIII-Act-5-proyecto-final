...
if (isset($_GET['q'])) {
    ...
    $nombre = $_GET['pro'];
    $hoy = date('Y-m-d');
    $producto = mysqli_query($conexion, "SELECT * FROM productos WHERE codigo LIKE '%" . $nombre . "%' OR descripcion LIKE '%" . $nombre . "%' AND vencimiento > '$hoy' OR vencimiento = '0000-00-00'");
    ...
    $id = $_SESSION['idUser'];
    $datos = array();
    $detalle = mysqli_query($conexion, "SELECT d.*, p.codproducto, p.descripcion FROM detalle_temp d INNER JOIN productos p ON d.id_producto = p.codproducto WHERE d.id_usuario = $id");
    ...
    if ($insertar) {
        ...
        while ($row = mysqli_fetch_assoc($consultaDetalle)) {
            ...
            $stockActual = mysqli_query($conexion, "SELECT * FROM productos WHERE codproducto = $id_producto");
            $stockNuevo = mysqli_fetch_assoc($stockActual);
            $stockTotal = $stockNuevo['existencia'] - $cantidad;
            $stock = mysqli_query($conexion, "UPDATE productos SET existencia = $stockTotal WHERE codproducto = $id_producto");
        } 
        if ($insertarDet) {
    ...
    $id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM productos WHERE codproducto = $id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
    ...
    $id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM armazon WHERE id = $id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
} else if (isset($_GET['editarLab'])) {
    $id = $_GET['id'];
    $sql = mysqli_query($conexion, "SELECT * FROM sellos WHERE id = $id");
    $data = mysqli_fetch_array($sql);
    echo json_encode($data);
    exit;
}
